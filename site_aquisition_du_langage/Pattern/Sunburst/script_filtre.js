...
export default function define(runtime, observer) {
    ...
    const fileAttachments = new Map([
        ["pattern.json", new URL("./files/data_filtre",
            import.meta.url)]
    ]);
    ...
    main.variable("data").define("data", ["FileAttachment"], function(FileAttachment) {
        return (
            FileAttachment("pattern.json").json()
        )
    });
    main.variable("partition").define("partition", ["d3"], function(d3) {
        return (
            data => {
    ...
    main.variable("format").define("format", ["d3"], function(d3) {
        return (
            d3.format(",d")
        )
    });
    main.variable("width").define("width", function() {
        return (
            932
        )
    });
    main.variable("radius").define("radius", ["width"], function(width) {
        return (
            width / 6
        )
    });
    main.variable("arc").define("arc", ["d3", "radius"], function(d3, radius) {
        return (
            d3.arc()
            .startAngle(d => d.x0)
            .endAngle(d => d.x1)
    ...
    main.variable("d3").define("d3", ["require"], function(require) {
        return (
            require("d3@5")
        )
    });
